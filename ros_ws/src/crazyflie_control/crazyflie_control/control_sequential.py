...
class CrazyflieController(Node):
    ...
    def apply_control(self, scf: Swarm, controller_cf: dict):
    ...
    def control_callback(self):
        ...
        if self.flight_phase == "takeoff":
            ...
            if self.current_step == len(self.refto[self.uris[0]]):
                ...
                self.current_step = 0  # Reset step for normal trajectory tracking
            ...
            if done == 1:
                cfcontrol.save_data(f"src/Data_Drone/data_files/drone_data_{self.simid}.npy", self.output_data, self.ref, self.vref)
                cfcontrol.save_data(f"src/Data_Drone/data_files/drone_data_{self.simid}_takeoff.npy", self.takeoff_data, self.refto, self.vrefto)
                np.save(f"src/Data_Drone/data_files/computation_times_{self.simid}.npy", {"comp_times": self.comp_times})
    ...
    def control_loop(self, cnt, ref_set, vref_set):
        ...
        for i, drone in enumerate(self.drone_bodies):
            ...
            # if self.flight_phase == "takeoff":
            #     v = cfcontrol.compute_land_control(self.state[drone][:, 0:6], ref_set[self.uris[i]][self.current_step].reshape(1, -1), self.Kp, self.Kd)
            #     cf_control[self.uris[i]] = [cfcontrol.get_cf_input(v, yaw_tmp, T_coeff=self.T_coeff[i], alpha=self.alpha[i], mass=self.m[i])]
        ...
        # v = self.PDcontroller.compute_control_real(state_xi.T, ref_set[self.uris[i]][cnt].reshape(1, -1), cnt, id=0)
        # v = compute_control_real(self.solver_data, state_xi.T, ref_set[self.uris[i]], vref_set[self.uris[i]], cnt, id=0)
        # v = cfcontrol.compute_control(vref_set[self.uris[i]][cnt, :].reshape(-1, 1), state_xi, ref_set[self.uris[i]][cnt].reshape(-1, 1), self.Kf[self.uris[i]])
        ...
        # self.controls[drone] = cfcontrol.get_real_input(v, yaw_tmp)
            ...
            currentTz[self.uris[i]] = [cf_control[self.uris[i]][0][3], self.state[drone][:, 2][0]]  # Saving the current Thrust and height to gradually land
